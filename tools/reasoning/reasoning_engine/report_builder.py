...
def render_check_human(report: dict) -> str:
    lines = [
        ...
        f"- Historical reproducibility: `{report['checks'].get('historical_reproducibility', 'not_reported')}`",
        f"- Current eligibility: `{report['checks'].get('current_eligibility', 'not_reported')}`",
        f"- Eligibility checked at: `{report['checks'].get('eligibility_checked_at', 'not_reported')}`",
